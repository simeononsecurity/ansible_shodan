...
def get_shodan_host_count(api, query, facet, minify):
    api_string = f"?key={api}"
    query_string = f"&query={query}"
    minify_string = f"&minify={minify}"
    facet_string = f"&facets={facet}" if facet else ""
    ...
    if not api:
        ...
        response = requests.get(f"https://api.shodan.io/shodan/host/count{api_string}{query_string}{facet_string}{minify_string}")
        if response.status_code == 200:
            return response.json()
        else:
            return f"Failed to get host count. Status code: {response.status_code}"
...
def main():
    ...
    result = get_shodan_host_count(api_key, query, facet, minify)
